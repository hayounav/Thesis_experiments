/** A lock-based concurrent skiplist.
  ...
  * This version assumes hash values do not collide. */
...
class LazySkipList[T] extends Set[T]{
  ...
  /** Search for item and key.
    ...
    * (3) if item is in the list at level (i), it's in succs(i).
  ...
  private def find(item: T, key: Int) : (Int, Array[Node], Array[Node]) = {
	...
	pred = curr; curr = pred.next.get(level)
      }
      if(curr.key == key) assert(curr.item == item)
      if(lFound == -1 && curr.key == key) lFound = level

...
/** A class representing a set of As, using sharding and synchronized 
  ...
  */
class ShardedSet[A: scala.reflect.ClassTag](shards: Int, initSize: Int) 
extends HashSet[A]{
  ...
  // {keys(i)(j) | i <- [0..shards), j <- [0..keys(i).size),
  //               stored(hashes(i)(j)) }.
  ...
  def putIfAbsent(key: A) : Boolean = {
    ...
    locks(sh).synchronized{
      ...
      if(hs(i) == store(h)) false
      else{ 
	numEntries(sh) += 1; ks(i) = key; hs(i) = store(h); true
      }
    }
  }
  ...
  private def resize(sh: Int) = {
	...
	val j = findPlace(newKs, newHs, ks(i), hashMask(hs(i)))
        newKs(j) = ks(i); newHs(j) = hs(i)
	count += 1
      }
      i += 1
    }
    // Set new table
    keys(sh) = newKs;  hashes(sh) = newHs
